...
DESIGNFACTOR <- createDesign(
  ...
  lev1_vp = c(1, 2, 3),
  lev2_vp = c(1, 2, 3),
  # variance of random intercepts and random slopes, 1= equal relationship, 
  # 2 = variance of random intercepts > random slopes
  var = c(1,2)
)
...
Generate <- function(condition, fixed_objects = NULL) {
  ...
  ## variance of random intercepts and variance of random slopes
  if(condition$var == 1) {
    tau1 = tau0
    tau1sq = tau1 ^ 2
...
# test
# dat <- Generate(DESIGNFACTOR[1,])
# dat30 <- Generate(DESIGNFACTOR[2,])
...
# Helper functions for fitting random slopes model with Kenward Roger
...
Analyse <- function(condition, dat, fixed_objects = NULL) {
  # Initialize output
  methods <- c("REML", "Kenward-Roger", "Adjusted CRVE RI", "Adjusted CRVE RS", "Adjusted CRVE OLS")
...
# test
# results <-data.frame(rbind(Analyse(DESIGNFACTOR[1,], dat15),Analyse(DESIGNFACTOR[2,], dat30)))
