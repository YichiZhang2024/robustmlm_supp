...
# Helper functions for fitting MLM
...
Analyse <- function(condition, dat, fixed_objects = NULL) {
  ...
  methods <- c("Kenward-Roger", "Adjusted CRVE RI", "Adjusted CRVE RS", "Adjusted CRVE OLS")
...
res <- runSimulation(design = DESIGNFACTOR,
                     ...
                     filename = "sim10_trial1",
                     seed = rep(2342, nrow(DESIGNFACTOR)),
                     #allow_na = TRUE,
                     parallel = TRUE,
                     save = TRUE,
                     save_results = TRUE
)
saveRDS(res, "sim10_trial1.RDS")